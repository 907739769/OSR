...
- **日志级别的唯一来源是 `logback-spring.xml`，`application.yml` 里不要再配 `logging.level.*`**。Spring Boot 的 `logging.level.*` 在 logback 配置加载**之后**应用，两边写了同一个 logger 时永远是 yml 赢。踩过一次：yml 写 `com.osr: debug`、logback 写 `<logger name="com.osr" level="info"/>`，于是整个 `com.osr.*`（连带 osr-system、osr-framework、osr-common）实际跑在 DEBUG，而读 logback 配置的人会得出完全相反的结论——两份配置各自都「看起来是对的」。临时排查用环境变量 `LOGGING_LEVEL_COM_OSR=debug` 覆盖，不必改文件。同一个文件里另外三条：**`file_debug` 用 `LevelFilter` 只收 DEBUG 而不是 `ThresholdFilter`**（后者是「DEBUG 及以上」，会让每条 INFO/WARN 同时写进 sys-debug.log 和 sys-info.log、每条 ERROR 写三份，凭空多一倍磁盘写入，还让 sys-debug.log 更快滚掉——而它恰恰是排查时最想往回翻的那个）；**三个文件 appender 外面包 `AsyncAppender`，且 info/error 必须设 `discardingThreshold=0`**（默认值会在队列 80% 满时丢弃 INFO 及以下，而 sys-error.log 是本项目排查线上异常的唯一依据，在最需要它的时刻悄悄丢几条比慢一点糟得多；debug 那个留默认，洪水时丢 debug 正是想要的降级方向）；**dev profile 的 `<root>` 只能挂 console**，`appender-ref` 是追加不是替换，再挂一遍底层 `file_*` 会让同一条日志经由 async 和直连两条路径写进同一个文件，每行出现两次。`includeCallerData` 保持默认 false 是安全的：三个文件 appender 的 pattern 都不含 `%method/%line`（dev 那个带行号的 pattern 是在这些 appender 定义之后才覆盖的，只对 console 生效）。
