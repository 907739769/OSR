...
- **高频探针端点必须同时进 `ApiInterceptor#SKIP_PATHS` 与 `RequestLogFilter#NO_LOG_PATHS` 两份清单，漏一处就只减半。** 一次请求会在这两个组件各留两行（各自的进入 + 收尾），`/api/health` 被 healthcheck 每 15 秒打一次，就是 **4 行 × 5760 次 = 每天 23040 行**——实测它在一份 1085 行的样本里独占 **60.5%**，把真正的业务日志挤到 3%。**`RequestLogFilter` 那侧不要图省事塞进 `EXCLUDE_PATHS`**：那个清单命中后会走「非 GET 一律 405」的静态资源分支，把真实 API 端点放进去会连带改掉它的请求语义；`NO_LOG_PATHS` 只是个布尔标志、不提前 return，因为 traceId 的初始化与 `MDC.clear()`、请求体缓存都在那段 try/finally 里，跳过整段会让这些请求丢掉 traceId。两个组件的职责本身是重复的（方法/URI/耗时两边都记，只有参数·body 与 IP·UA·状态码是各自独有的），合并是笔待还的债，但那要先定清楚「一条请求日志该长什么样」。
