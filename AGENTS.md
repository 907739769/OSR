...
- **不要记「缓存命中」这类正常路径上的高频事件**。`TmdbCacheService` 曾对每次命中打一行 DEBUG，实测一次对账 236 行、占全量日志的 **22%**，而它零诊断价值——没人会因为「命中了」去查什么。未命中那一侧由「缓存写入」间接标记（未命中必然回源并回填），想看命中率应该走指标而不是逐条日志。判断标准是「这条日志会让谁在什么时候做什么决定」，答不上来就别打。
