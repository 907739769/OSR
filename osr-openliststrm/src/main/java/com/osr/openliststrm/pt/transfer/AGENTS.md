...
- **定时任务与「立即执行」共用 `TorrentTransferService#runRule` 里的按规则锁（`runningRules`）**。两边原先各有各的防重入——任务的 `running` 标志、按钮的 loading——互相看不见，同一条规则被两个线程同时评估会对同一批种子各发起一次转移（目标端「加种前查同 hash」能挡住大部分，但会多出一条误判的失败记录）。撞锁抛 `RuleBusyException`：定时任务 `transferAll` 遇到它只记 info 让路，**不能当成失败记 warn**；手动执行的端点在入口先 `isRunning` 判一次直接拒绝。手动执行**转后台跑、经 WebSocket 推 `transferRun` 事件**（同下载记录的批量重试），前端按 `runId` 认领——一轮「导出 → 加种 → 排除文件 → 触发校验」乘以单轮上限，同步等必然超过前端 15 秒的请求超时，而那时后端还在搬。
- **保存前校验走 `BaseCrudRestController#validateWrite` 钩子**：源与目标同一个下载器、源是 Transmission、体积上下限颠倒、路径映射不是合法 JSON（`PathMapping.validate`）都在保存时拦下。`PathMapping.parse` 运行期对坏配置是**刻意宽容**的（退化成不映射），两者不矛盾：运行期宽容是为了不让一条坏配置拖垮整条规则，保存时严格是为了让写错的 JSON 不被静默当成「不映射」。
- **清除失败记录（`DELETE /pt-transfer-records/failed`）限管理员**，与改规则、立即执行同级：清掉等于解除「停止重试」的闸门，那批种子会重新开始转移，最后一步是删源端种子。前端点按钮前先查一次失败条数再弹确认——从工具栏打开记录弹窗时不带 `ruleId`，清的是**全部规则**。
