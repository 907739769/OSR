...
- **保种与上传量看板（`SeedingStatsService` + `PtSeedingRestController`，统计仪表盘底部的 `components/SeedingPanel.vue`，两端共用）是这组统计里唯一不做归属隔离的——所以整块只给管理员**。数据来自下载器本身，一台下载器里是全家所有人的种子，没有按订阅归属拆分的口径；非管理员请求返回 403，前端据此静默藏起整块（`silent` 请求，不弹「仅管理员」的报错）。三条：**每日上传量按下载器的累计计数器求差**（`IDownloaderClient#cumulativeUploaded`：qB `sync/maindata` 的 `alltime_ul`、TR `session-stats` 的 `cumulative-stats.uploadedBytes`），**不按现存种子的 `uploaded` 求和**——删一个上传过 50G 的种子那天就是 -50G；计数器取不到才用求和兜底，**任一口径算出负数（删种、计数器被清零）那天当作算不出、画成空柱而不是 0**。**快照一天一行、每小时覆盖**（`SeedSnapshotTask` → `pt_seed_snapshot`，20260807），最后一次成功的即当日收盘值——一天只拍一次的话赶上那一刻下载器离线，那天就整个缺了。现状部分（各下载器保种数/体积、上传最多、低效保种）每次打开页面实时拉 `listAll`，不走快照；「低效保种」= 做种满 7 天且分享率 < 0.2、按体积降序，给删种决策参考，**这里只展示不删**，删种仍走 `pt/clean` 那套显式开启的规则。
