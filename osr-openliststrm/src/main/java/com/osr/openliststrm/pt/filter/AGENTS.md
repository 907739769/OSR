...
- **自然语言建规则（`FilterRuleAiDraftService`，`POST /pt-filter-config/ai-draft`，仅管理员）只产出草稿、绝不落库**：前端把 `changes` 合进表单，页面随之进入「有未保存修改」，用户看过（可以先跑历史回放）再自己点保存。AI 的输出一律当不可信输入：**只认白名单字段**（排序维度顺序、补搜周期这类不开放）；**分辨率/来源/标签只保留 `FilterVocabulary` 里有的值并归一大小写**——后端按全等比对，词表外的值存进去永远命不中、比不改还糟（与前端「规则类多选一律 v-select 不用 combobox」同一个理由）；部分合规时保留合规的，全部不合规整字段不改；**丢掉的必须写进 `dropped` 回给页面**，否则用户会以为「不要杜比全景声」那半句已经生效。体积让 AI 按 GB 给、这里换成字节，前端再按 `SIZE_FIELDS` 换回 GB 填表单。草稿以**表单当前值**（可能未保存）为基础，不是库里那份。
